# Laboratory Adaptive Optimization Validation Platform MVP

这是一个本地可运行的实验室自适应优化验证平台原型。当前版本不依赖外部服务，使用 Python 标准库后端和原生 HTML/CSS/JS 前端，适合快速验证实验类型接入、数据导入、指标提取、优化推荐、仿真验证、插件管理和报告导出流程。
